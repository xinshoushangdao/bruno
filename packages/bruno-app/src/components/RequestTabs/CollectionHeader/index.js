...
const CollectionHeader = ({ collection, isScratchCollection }) => {
  ...
  const displayName = isScratchCollection
    ? (currentWorkspace?.name || t('COMMON.UNTITLED_WORKSPACE'))
    : (collection.name || t('COMMON.UNTITLED_COLLECTION'));
  ...
  const overflowMenuItems = [
    { id: 'variables', label: t('SIDEBAR.VARIABLES'), leftSection: IconEye, onClick: viewVariables },
    ...
    ...(isOpenAPISyncEnabled && !hasOpenApiSyncConfigured
      ? [{ id: 'openapi-sync', label: t('SIDEBAR.OPENAPI'), leftSection: OpenAPISyncIcon, rightSection: <StatusBadge status="info" size="xs">{t('SIDEBAR.BETA')}</StatusBadge>, onClick: viewOpenApiSync }]
      : []),
    { id: 'collection-settings', label: t('SIDEBAR.COLLECTION_SETTINGS'), leftSection: IconSettings, onClick: viewCollectionSettings }
  ];
  ...
  const handleExportWorkspace = () => {
    ...
    dispatch(exportWorkspaceAction(uid))
      .then((result) => {
        if (!result?.canceled) {
          toast.success(t('SIDEBAR.WORKSPACE_EXPORTED'));
        }
      })
      .catch((error) => {
        toast.error(error?.message || t('SIDEBAR.ERROR_EXPORTING_WORKSPACE'));
      });
  };
  ...
  const validateWorkspaceName = (name) => {
    ...
    if (!trimmed) {
      return t('COMMON.NAME_IS_REQUIRED');
    }
    if (trimmed.length > 255) {
      return t('COMMON.MUST_BE_255_CHARS_OR_LESS');
    }
    return null;
  };
  ...
  const handleSaveWorkspaceRename = () => {
    ...
    if (!trimmedName) {
      ...
      setWorkspaceNameError(t('COMMON.NAME_IS_REQUIRED'));
      return;
    }
    ...
    if (currentWorkspace?.isCreating) {
      dispatch(confirmWorkspaceCreation(uid, trimmedName))
        ...
        .catch((err) => {
          toast.error(err?.message || t('SIDEBAR.ERROR_CREATING_WORKSPACE'));
        })
        .finally(() => {
      ...
      dispatch(renameWorkspaceAction(uid, workspaceNameInput))
        .then(() => {
          toast.success(t('SIDEBAR.WORKSPACE_RENAMED'));
          setIsRenamingWorkspace(false);
          setWorkspaceNameInput('');
          setWorkspaceNameError('');
        })
        .catch((err) => {
          toast.error(err?.message || t('SIDEBAR.ERROR_RENAMING_WORKSPACE'));
          setWorkspaceNameError(err?.message || t('SIDEBAR.FAILED_TO_RENAME_WORKSPACE'));
        })
        .finally(() => {
          isSavingRef.current = false;
        });
            ...
            <div className="flex flex-row justify-center items-center gap-x-1">
              ...
              >
                <div className="max-w-124 overflow-hidden">
                  {currentWorkspace && (
                    <>
                      <div className="label-item">{t('SIDEBAR.WORKSPACE')}</div>
                      <div
                  ...
                  {mountedCollections.length > 0 && (
                    <>
                      ...
                      <div className="label-item">{t('SIDEBAR.COLLECTIONS')}</div>
                      {mountedCollections.map((col) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="dropdown-label collection-header-dropdown-label">{col.name || t('COMMON.UNTITLED_COLLECTION')}</span>
                            {colTabCount > 0 && (
        ...
        <div className="flex flex-grow gap-1.5 items-center justify-end">
          {!isScratchCollection && (
            <>
              ...
              {isOpenAPISyncEnabled && hasOpenApiSyncConfigured && (
                <ToolHint
                  text={hasOpenApiError ? t('SIDEBAR.OPENAPI_ERROR') : hasOpenApiUpdates ? t('SIDEBAR.OPENAPI_UPDATES_AVAILABLE') : t('SIDEBAR.OPENAPI')}
                  toolhintId="OpenApiSyncToolhintId"
                  place="bottom"
                >
                  <ActionIcon onClick={viewOpenApiSync} aria-label={t('SIDEBAR.OPENAPI')} size="sm" className="relative">
              ...
              <ToolHint text={t('SIDEBAR.RUNNER')} toolhintId="RunnerToolhintId" place="bottom">
                <ActionIcon onClick={handleRun} aria-label={t('SIDEBAR.RUNNER')} size="sm" data-testid="runner">
              ...
              <MenuDropdown items={overflowMenuItems} placement="bottom-end" data-testid="more-actions">
                <ActionIcon label={t('SIDEBAR.MORE_ACTIONS')} size="sm" style={{ border: `1px solid ${theme.border.border1}`, borderRadius: theme.border.radius.base, width: 24, marginRight: 4, marginLeft: 4 }}>
